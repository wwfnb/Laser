...
class InferenceActor:
    ...
    def initial_batch(self, samples: list) -> dict:
        ...
        for sample in samples:
            ...
            assistant_msg_1 = bulid_assistant_msg(sample["raw_response"][0])
            user_msg_2_1 = bulid_user_msg_2(resize_first_crop_image)
            assistant_msg_2 = bulid_assistant_msg(sample["raw_response"][1])
            user_msg_2_2 =  bulid_user_msg_2(resize_second_crop_image)
    ...
    def infer(self, samples: list, sampling_params: SamplingParams, batch_size: int= 8, max_roll: int= 2):
...
def train_data_reader(max_samples: int= 20_000):
    while True:
        ...
        host_name = os.uname().nodename  # 等价于 $(hostname)
        print("[INFO]host_name:", host_name)
        ...
        if host_name == "gpu08":
            lines_to_read = all_lines[:half_index]
            print("[INFO] gpu08 读取前一半")
        elif host_name == "gpu09":
            lines_to_read = all_lines[half_index:]
            print("[INFO] gpu09 读取后一半")
        else:
            raise RuntimeError(f"未识别主机名: {host_name}，请在代码中指定读取规则")
        print(f"[INFO] load from [{DATA_PATH}]")
        ...
        for line in lines_to_read:
            ...
            if item["for_trible_turn"]['id'] in existed_ids: continue
            data = copy.deepcopy(item["for_trible_turn"])

            pre_a100_url = f"/public/home/ljt/hqp/GUI-grounding/train_data/grounding-r1"
            pre_4090_url = f"/data/hqp/GUI-grounding/train_data"

            if data["img_path"].startswith(pre_4090_url):
                data["img_path"] = data["img_path"].replace(pre_4090_url,pre_a100_url)

            if data["img_filename"].startswith(pre_4090_url):
                data["img_filename"] = data["img_filename"].replace(pre_4090_url, pre_a100_url)
