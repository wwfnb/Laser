...
##correctness, pre_box, raw_response = eval_sample_positive_gt(sample["img_size"], sample["resized_img_size"], sample["raw_response"], sample["bbox"])

##只选第一个格式正确的,有些没有以</box>结尾的，给其加了

##没有限制框的最大值，即模型输出很大的框，只要包裹住就正确，但是排除掉超出屏幕的了
# def eval_sample_positive_gt(raw_point_lst, bbox):
#     if len(raw_point_lst) == 0 or all(x is None for x in raw_point_lst):
#         return "wrong_format"
...
def eval_data_reader(args) -> list[dict]:
    print("loading data!")
    if args.task == "all":
        task_filenames = [
            ...
            for gt_type in gt_types:
                for lang in languages:
                    for task_instance in task_data:
                        ...
                        ###绝对路径
                        filename = task_instance["img_filename"]
                        task_instance["img_filename"] = os.path.join(args.screenspot_imgs, filename)
                        if lang == "cn":
...
def parse_args():
    ...
    parser.add_argument('--min_pixels', type=int, required=True, default=2408448, help="Minimum number of pixels for resizing images.")
...
def smart_size_all_raw(tasks_to_run:list[dict], max_pixels:int):
    ...
    all_smart_image_dir = "cache/screenspot_pro_resize_image"
...
### TODO: 并行加载图片， 不用也行，只需要加载一次
# def parallel_smart_size(tasks_to_run:list[dict], max_pixels:int, max_workers:int=20):
...
#         futures = [executor.submit(smart_size_all_raw, chunk, max_pixels) for chunk in chunks]
#     output_lst = []

#     ###保持顺序
#     for future in futures:
#         output_lst.extend(future.result())
#     assert len(output_lst) == len(tasks_to_run)
#     return output_lst

def batch_inf(args):
    setup_ray()  ##debug
    tasks_to_run = eval_data_reader(args)
    ...
    for sample in infer_results:
        ...
        if sample["gt_type"] == "positive":
            ####用到 img_size, resized_img_size, raw_response(提取pre_box, 转换为raw), bbox
            n_focus_correctness, tryn_results, correctness = eval_sample_positive_gt(sample["trajs"], sample["bbox"])
        elif sample["gt_type"] == "negative":
            correctness = eval_sample_negative_gt(sample, sample)
        else:
